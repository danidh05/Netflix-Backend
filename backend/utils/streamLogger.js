...
// Define the log directory and file path for streaming logs
...
// Create a write stream (in append mode) for the streaming logs
const streamLogStream = fs.createWriteStream(logPath, { flags: "a" });

/**
 * Custom logger function to write streaming-specific logs to the console and a file.
 * @param {string} message - The log message to be recorded.
 */
export const streamLogger = (message) => {
  ...
  // Write to the file
  streamLogStream.write(formattedMessage);
};
